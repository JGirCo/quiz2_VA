...
def binary(image, method=0, Umin=0, Umax=255) -> MatLike:
    if method == 0:
...
def binaryHSV(image, minH=0, maxH=255, minS=0, maxS=255, minV=0, maxV=255) -> MatLike:
    imgBinary = cv2.inRange(image, (minH, minS, minV), (maxH, maxS, maxV))
    return imgBinary


def frame_contours(frame: MatLike, umin: int, umax: int):
...
def create_trackbar():
    ...
    cv2.createTrackbar("max", "Placa", 75, 255, lambda x: None)
...
def main() -> None:
    ...
    while True:
        ...
        if frame is not None:
            ...
            frame_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            frame_denoised = cv2.fastNlMeansDenoising(frame_gray, None, 10, 10, 21)
            frame_bin = binary(frame_denoised, method=1, Umin=min, Umax=max)
